...
//
// KEEP IN SYNC with `functions/src/utils/cidr.ts`, which this mirrors verbatim.
// The client copy exists so Settings validates allowlist entries by exactly the
// rules the server matches by — a divergence here would let an admin save a
// prefix the backend then silently skips. Duplicated rather than shared for the
// same reason as `searchTokens.ts` and `passwordGenerator.ts`: the frontend and
// functions are separate TypeScript projects with separate roots.
//
